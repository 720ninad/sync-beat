...
export async function uploadTrack(req: Request, res: Response) {
  try {
    ...
    if (!allowedTypes.includes(req.file.mimetype)) {
      res
        .status(400)
        .json({ error: "Only audio files are allowed (mp3, wav, ogg, m4a)" });
      return;
    }
...
export async function likeTrack(req: Request, res: Response) {
  try {
    ...
    const [existing] = await db
      ...
      .where(
        and(
          eq(likedTracks.userId, req.user!.id),
  ...
  try {
    ...
    if (track.fileUrl && track.mimeType !== "external") {
      try {
        ...
        console.error(
          "R2 deletion error (continuing with DB deletion):",
          r2Error,
        );
...
async function searchFromMultipleSources(
  query: string,
): Promise<ExternalTrack[]> {
  try {
    const results = await searchJioSaavn(query, 20);
    return results
      .map(mapToTrack)
...
function mapToTrack(song: {
  ...
  // Skip songs where we couldn't derive a playable URL
  ...
  return {
    ...
    image: song.image,
    preview_url: song.url, // direct CDN URL — plays and downloads directly
    external_id: song.id,
    source: "jiosaavn",
  };
}
